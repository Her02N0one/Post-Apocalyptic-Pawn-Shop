"""scenes/world/update.py — Extracted helpers for TopDown.

Functions that previously lived as methods on TopDown.  Each takes
a ``scene`` parameter (the TopDown instance) so they can read/write
...
def route_ui_event(scene, event: pygame.event.Event, app):
    ...
    for cmd in cmds:
        if isinstance(cmd, CloseModal):
            ...
            from systems.actions import open_npc_trade
...
def _apply_intent(scene, intent):
    ...
    from systems.actions import OpenDialogueIntent, OpenTransferIntent, OpenInventoryIntent
