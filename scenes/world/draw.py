"""scenes/world/draw.py — Rendering helpers for the world scene.

All pure-draw functions live here so that TopDown.draw() stays thin.
...
def draw_debug_overlay(surface: pygame.Surface, app: App, scene, cam: Camera):
    ...
    for eid, npc_pos, ident in app.world.query(Position, Identity):
        ...
        if brain:
            ...
            # CombatState and VillagerState have dict-compat .get(),
            # so use that for uniform access.
            villager_st = brain.state.get("villager", {})
            _path_sources = [
                (combat_state, "_path", (255, 120, 50)),             # combat chase
                (villager_st, "_path", (50, 255, 120)),              # villager walk
                (villager_st, "_vw_path", (120, 200, 255)),          # villager wander
                (brain.state, "_w_path", (200, 120, 255)),           # wander brain
            ]
            for src_dict, key, path_color in _path_sources:
                apath = src_dict.get(key) if hasattr(src_dict, 'get') else None
