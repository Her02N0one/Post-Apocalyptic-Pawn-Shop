...
class ZooScene(Scene):
        ...
        for idx, (entry_id, entry_data) in enumerate(self._entries):
            ...
            if self.mode == "characters":
                ...
                try:
                    ...
                    if brain:
                        brain.active = False  # frozen in zoo
    ...
    def handle_event(self, event: pygame.event.Event, app: App):
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_TAB:
                ...
                self._populate(app)
            if event.key == pygame.K_RETURN and self._selected_eid is not None:
                # Could expand to a detail view
                pass
    ...
    def _try_select(self, app: App):
        ...
        world_x = (mx - ox) / TILE_SIZE
        world_y = (my - oy) / TILE_SIZE

        best_eid = None
        best_dist = 2.0  # max click distance in tiles
        for eid in self._eids:
    ...
    def draw(self, surface: pygame.Surface, app: App):
        ...
        # ── Header ───────────────────────────────────────────────────
        mode_label = "CHARACTERS" if self.mode == "characters" else "ITEMS"
        count = len(self._entries)
        header = f"ZOO: {mode_label} ({count} entries)  [Tab] switch  [Esc] back"
