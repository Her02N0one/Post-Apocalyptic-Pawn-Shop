...
Public API
----------
``is_guard``           — True if entity is combat-capable + non-hostile
...
from components import Identity, Faction
from components.ai import Brain, Threat, AttackConfig
from core.tuning import get as _tun


def is_guard(world: Any, eid: int) -> bool:
    """True if entity is combat-capable and non-hostile.

    Guard = has AttackConfig (armed) + Faction with friendly disposition.
    Used by crime, checkpoint, communal meal systems.
    """
    if not world.has(eid, AttackConfig):
        return False
    faction = world.get(eid, Faction)
    return faction is not None and faction.disposition == "friendly"
