...
``systems.faction_disposition`` for all faction mutations.
...
from components import (
    Player, Position, Health, Faction,
    CrimeRecord,
)
...
def find_witnesses(world: World, zone: str, thief_x: float,
...
def report_theft(world: World, witnesses: list[int], item_id: str,
...
def make_theft_callback(world: World, owner_faction: str,
...
def npc_knows_crimes(world: World, npc_eid: int,
...
def guard_crime_reaction(world: World, guard_eid: int,
                         game_time: float) -> bool:
    ...
    """
    if not is_guard(world, guard_eid):
        return False
...
def make_lockpick_callback(world: World, locked_comp,
                           owner_faction: str,
                           game_time_fn: callable) -> callable:
