"""systems/combat/fireline.py — Fire-line awareness and communication.
...
from components import Brain, AttackConfig
from core.tuning import get as _tun
from systems.combat.allies import iter_same_faction_allies
...
def get_ally_fire_lines(world: World, eid: int, pos) -> list[FireLine]:
    ...
    for aid, apos in iter_same_faction_allies(world, eid, pos):
        ...
        if not isinstance(ac, CombatState) or ac.mode != "attack":
            continue
        tgt = ac.p_pos
        if tgt is None:
            continue
...
def request_clear_fire_line(world: World, blocker_eid: int,
    ...
    if not isinstance(c, CombatState):
        return
    if c._tac_repos:
        return
    c._clear_fire_line = {
