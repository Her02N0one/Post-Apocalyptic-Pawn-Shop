"""systems/combat/alerts.py — Sound alerts, intel sharing, and faction flipping.
...
and delegate faction/flee mutations to ``systems.faction_disposition``.
"""

from __future__ import annotations

from components import (
    Brain, Faction, Position,
...
def alert_nearby_faction(world, defender_eid: int, attacker_eid: int):
    ...
    for eid, apos, _af, dsq in world.nearby(
        ...
        if ally_brain:
            cs = ally_brain.state.get("combat")
            if isinstance(cs, CombatState) and cs.mode in ("chase", "attack", "flee"):
                continue
