"""systems/combat/engagement.py — Combat AI orchestrator.
...
from systems.ai.perception import hp_ratio, should_engage
from systems.ai.steering import face_toward, face_from_velocity
from systems.ai.defense import try_dodge, try_heal, reset_faction_on_return
from systems.ai.brains import _log
from systems.combat import targeting as sense
from systems.combat import movement as move
from systems.combat.state import get_combat_state
from systems.combat.fireline import get_ally_fire_lines, fire_line_dodge_vector, request_clear_fire_line
from systems.combat.tactical import find_tactical_position, find_chase_los_waypoint
from systems.combat.allies import PointProxy
...
def _run_sensor_tick(world, eid, brain, pos, vel, patrol, threat, atk_cfg,
    ...
    if c.mode in ("chase", "attack") and target.eid is not None:
        from systems.combat.alerts import share_combat_intel
...
def _try_attack(world, eid, pos, atk_cfg, c, target, is_ranged, game_time):
    ...
    if c.attack_until > game_time or target.eid is None:
        return
    ...
    if is_ranged:
        ...
        if target.ally_in_fire:
            c._los_blocked = True
            blocked = c._los_blocked_count + 1
...
def _emit_attack(world, attacker_eid, target_eid, attack_type):
    ...
    if bus:
        ...
        from systems.combat.attacks import npc_ranged_attack, npc_melee_attack
...
def _search_rotate_facing(world, eid, pos, c, game_time):
    ...
    if dist_to_src < 2.0:
        ...
        start = c._search_start or game_time
...
def _maybe_start_tactical_repos(world, eid, pos, atk_cfg, c, target,
    ...
    if c._tac_repos and c._tac_repos_until > game_time:
        return
    ...
    callout = c._clear_fire_line
    c._clear_fire_line = None
...
def _run_movement(world, eid, pos, vel, patrol, atk_cfg,
    ...
    if mode == "idle":
        ...
        face_from_velocity(world, eid, vel)

    elif mode == "searching":
        src = c.search_source or (pos.x, pos.y)
        ...
        if is_ranged:
            ...
            move.ranged_attack(
                ...
                los_blocked=c._los_blocked,
                game_time=game_time,
                repos_target=c._repos_target,
            )
        else:
