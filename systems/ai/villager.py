"""systems/ai/villager.py — Schedule-driven village NPC brain.
...
from systems.ai.perception import find_player
...
# Canonical eat logic lives in ``systems.inventory_consume.npc_try_eat_any``.
# Previously this file had its own _consume_food — removed to avoid
# duplication of the eat-from-inventory algorithm.
...
def _villager_brain(world: World, eid: int, brain: Brain, dt: float,
    ...
    s = brain.state
    v = get_villager_state(brain)
    if v.origin is None:
        v.origin = (pos.x, pos.y)
    if not v.home_subzone:
        v.home_subzone = ""
...
def _wander_step(patrol: HomeRange, pos, vel, s: dict, dt: float,
    ...
    """Thin wrapper around :func:`systems.ai.wander.wander_step`."""
    from systems.ai.wander import wander_step
