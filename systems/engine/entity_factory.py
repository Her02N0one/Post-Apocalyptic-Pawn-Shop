"""systems/entity_factory.py — Table-driven entity spawning.
...
# ── Shared archetype-completion helper ───────────────────────────────

def ensure_combat_components(world: World, eid: int) -> None:
    """Guarantee an entity has the physics/combat components its archetype needs.

    * Health → Collider + Hurtbox  (so it can be hit / take damage)
    * Brain  → Velocity            (so it can move)
    * Always → Facing              (facing direction, cheap to add)

    Safe to call repeatedly — only adds missing components.
    """
