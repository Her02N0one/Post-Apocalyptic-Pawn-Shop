    ...
    while running:
                ...
                # ── Tab: toggle 3D editor / 2.5D view ──
                    ...
                    if mod & pygame.KMOD_SHIFT:
                        ...
                        if view_mode == "2d":
                            ...
                            # If zone was modified, rebuild renderer buffers
                            if editor_3d.dirty or True:
                                renderer.update_zone(zone, atlas, dn)
        ...
        def try_move(dx: float, dy: float) -> None:
            ...
            if noclip:
                px, py = nx, ny
                return
            if renderer.can_move_to(nx, py):
                px = nx
            if renderer.can_move_to(px, ny):
                py = ny
        ...
        frame_surf = renderer.render(px, py, angle)
        if show_entities:
