...
class EditorWindow(QMainWindow):
    def __init__(self, zone: Zone, zone_name: str = "") -> None:
        ...
        self._entity_inspector.type_selected.connect(self._restore_viewport_focus)
        ...
        self._cell_dock.setMinimumWidth(180)
        ...
        self._minimap_dock.setMinimumWidth(180)
    ...
    def _build_toolbar(self) -> None:
        ...
        # Entity snap cycle button
        snap_act = QAction("Snap: Free", self)
        snap_act.setToolTip("Cycle entity snap mode (S)")
        snap_act.triggered.connect(self._cycle_entity_snap_toolbar)
    ...
    def _cycle_entity_snap_toolbar(self) -> None:
        ...
        self.statusBar().showMessage(f"Entity Snap: {snap_lbl}", 1500)
        self.viewport.update()
    ...
    def _show_entity_quick_search(self) -> None:
        ...
        popup.move(
            vp_rect.x() + (vp_rect.width() - popup_w) // 2,
            vp_rect.y() + 40,
        )
        popup.accepted.connect(lambda: self.viewport.update())
        popup.show()
        popup.setFocus()
    ...
    def keyPressEvent(self, ev) -> None:
        ...
        # ── Below here: camera is NOT active ──
        ...
        # ── G key: cycle snap in sculpt, toggle grid otherwise ──
        if key == Qt.Key.Key_G:
            if is_sculpt:
        ...
        if is_entity:
            ...
            if key == Qt.Key.Key_BracketRight:
                etype = self._entity_tool.cycle_type(1)
                self._entity_inspector.select_type_in_tree(etype)
                self.statusBar().showMessage(f"Entity: {etype}", 1500)
                self.viewport.update()
                return
            if key == Qt.Key.Key_BracketLeft:
                ...
                return
            if key == Qt.Key.Key_F:
                self._show_entity_quick_search()
                return
    ...
    def _build_entity_overlays(self) -> list:
        ...
        for ent in zone.entities:
            ovls.extend(_entity_marker(
                ent, zone,
                is_selected=False, is_hovered=False, detailed=False,
            ))
        return ovls
    ...
    def _on_viewport_scroll(self, direction: int, mods: int = 0) -> bool:
        ...
        *mods* is the raw Qt keyboard modifier flags so we can dispatch
        ...
        if self._active_tool_name == "entity" and shift:
            ...
            degrees = round(math.degrees(step * direction))
...
class EntityQuickSearch(QWidget):
    ...
    def __init__(self, tool, inspector, parent=None) -> None:
        super().__init__(parent)
        self._tool = tool
        ...
        title = QLabel("<b style='color:#ccc;'>Quick Search Entities</b>")
        lay.addWidget(title)
    ...
    def _accept_item(self, item) -> None:
        ...
        if eid:
            self._tool.set_type(eid)
            self._inspector.select_type_in_tree(eid)
            self.accepted.emit()
        self.close()
